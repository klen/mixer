...
class TypeMixer(_.with_metaclass(TypeMixerMeta)):  # type: ignore
    ...
    def __repr__(self):
        return f"<TypeMixer {self.__scheme}>"
    ...
    def blend(self, **values):
        ...
        LOGGER.info("Blended: %s [%s]", target, self.__scheme)
        return target
...
class ProxyMixer:
    ...
    def blend(self, scheme, **values):
        ...
        for _ in range(self.count):  # noqa: F402
...
class Mixer(_.with_metaclass(_MetaMixer)):  # type: ignore
    ...
    def __repr__(self):
        return "<Mixer [{}]>".format("fake" if self.params.get("fake") else "rand")
    ...
    def blend(self, scheme, **values):
        ...
        try:
            ...
            if self.params.get("silence"):
                return None
            if e.args:
                e.args = (f"Mixer ({scheme}): {e.args[0]}",) + e.args[1:]
    ...
    def ctx(self, **params):
        ...
        """
        _params = {k: v for k, v in self.params.items() if k in params}
    ...
    def _guard(self, scheme, guards, **values):
        ...
        if seek:
            LOGGER.info("Finded: %s [%s]", seek, type(seek))
            return seek
